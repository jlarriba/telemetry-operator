...
type CloudKittyDefaults struct {
	...
	ProcessorContainerImageURL string
}
...
func (spec *CloudKittySpec) Default() {
	if spec.APIImage == "" {
		spec.APIImage = cloudKittyDefaults.APIContainerImageURL
	}
	if spec.ProcessorImage == "" {
		spec.ProcessorImage = cloudKittyDefaults.ProcessorContainerImageURL
	}
}
